...
Float_t leadlepPt, subleadlepPt, Ptll, MET, Mll, lepplusID, lepminID, Mt, METRel, WplusPt, WminPt, DPhill;



void plotVar(TString var,int nhyp, TString files[], TString type = "F"){
  bool debug = false;
  ...
  if(variable.Contains("onShell")) {xmin = 70; xmax=90;}
  if(variable.Contains("offShell")) {xmin = 0; xmax=70;}
  if(variable.Contains("DLepPt")) {xmin = 0; xmax=50;}
  if(variable.Contains("DPtcharge")) {xmin = -60; xmax=60; nbins = 30;}
  ...
  for(int i=0;i<nhyp;i++) {
    ...
    f_name = files[i];
    f_input = TFile::Open(f_name);
    t_input = (TTree*) f_input->Get("tree");
    ...
    else if(variable.EqualTo("DPhill")) {floatVar=DPhill;}
    ...
    t_input->SetBranchAddress("lepplusID", &lepplusID);
    t_input->SetBranchAddress("lepminID", &lepminID);
    t_input->SetBranchAddress("Mt", &Mt);
    t_input->SetBranchAddress("WplusPt", &WplusPt);
    t_input->SetBranchAddress("WminPt", &WminPt);
...
// returns -1 
  ...
  //if(goodEvent) {
    //if(WplusPt>WminPt) cout<<lepplusID<<endl;
    //else cout<<lepminID<<endl;
    //}
